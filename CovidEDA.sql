...
--Analyzing for country that has the highest infection rate vs population.
SELECT location, population, MAX(total_cases) AS HighestInfectioncount, MAX((total_cases/population)*100) AS TotalCovidPercentage
FROM CovidDeaths
WHERE total_cases IS NOT NULL AND continent IS NOT NULL
GROUP BY location, population
ORDER BY TotalCovidPercentage DESC

--Showing the countries that has the highest death count per population
